...
class NetworkMonitor:
    ...
    def __init__(self):
        ...
        self._last_status: Optional[Tuple[bool, bool]] = None
    ...
    async def check_online_api(self) -> bool:
        ...
        if not self.settings.OPENROUTER_API_KEY.startswith("sk-or-v1-"):
            logger.warning("⚠️ OpenRouter API key has invalid format")
            return False
        
        try:
            async with httpx.AsyncClient(timeout=self.settings.NETWORK_CHECK_TIMEOUT) as client:
                ...
                logger.info(f"🌐 Online API check: {'✅ Available' if is_available else '❌ Unavailable'} (status: {response.status_code})")
                return is_available
                
        except httpx.TimeoutException:
            logger.warning("⏱️ OpenRouter API timeout")
            return False
